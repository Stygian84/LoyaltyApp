    ...
    <Layout>
      <header>
        <nav className="flex h-20 items-centre px-3 justify-between shadow-md bg-white">
          <Link href="/">
            <a className=" text-lg font-bold text-black ml-10 mt-5">
              {user.credit_balance} available points
            </a>
          </Link>

          <div className="absolute ml-300   w-1 h-20 bg-gray-600"></div>
          <button className="absolute ml-300  px-4  h-20">
            Check Transaction status{" "}
          </button>
        </nav>
      </header>
      <h1 className="px-4 h-12 text-lg font-bold">Offers</h1>
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3 lg:grid-cols px-4">
        {prog.map((program) => (
